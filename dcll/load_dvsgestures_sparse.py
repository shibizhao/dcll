...
def compute_start_time(labels,pad):
    ...
    #print(tbegin, labels[l0[0],2], tend)
...
def create_data(filename = os.path.join(dcll_folder, '../../data/DvsGesture/dvs_gestures_events.hdf5'),
    ...
    strain = SequenceGenerator(filename=filename, group='train', batch_size = batch_size, chunk_size = chunk_size, size = size, ds = ds, dt= dt)
    stest  = SequenceGenerator(filename=filename, group='test', batch_size = batch_size, chunk_size = chunk_size, size = size, ds = ds, dt= dt)
    return strain, stest
